...
class StockPicking(models.Model):
    ...
    def action_ids_export(self):
        """Stock Picking listeden çağırıldığında CSV üret ve wizard aç."""
        wizard = self.env['stock.picking.export.wiz'].create({})
        wizard.generate_csv(self)
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'stock.picking.export.wiz',
            'view_mode': 'form',
            'res_id': wizard.id,
            'target': 'new',
        }
...
class StockPickingExportWiz(models.TransientModel):
    _name = 'stock.picking.export.wiz'
    _description = 'Stock Picking CSV Export Wizard'
    ...
    @api.model
    def generate_csv(self, picking_ids):
        ...
        for pckng in picking_ids:
            ...
            format_id = self._get_format_id(sale_id)
            delivery = pckng.partner_id
            company = main_company # tiskönig > delivery.company_id
            ref_company_ids = pckng.partner_id.parent_id.ref_company_ids
            reseller = self.env['res.partner']
            if ref_company_ids and main_company.id not in ref_company_ids.ids:
                reseller = pckng.partner_id.parent_id
            ...
            row_data = {
                ...
                'VORT': company.city or '',
                 # Reseller
                'NEMAIL': reseller.email or '',
                'NLAND': reseller.country_id.code or 'DE',
                'NNAME1': reseller.name or '',
                'NORT': reseller.city or '',
                'NPLZ': reseller.zip or '',
                'NSTRASSE': reseller.street or '',
                 # Delivery
                'NTELNR': delivery.phone or '',
                'NUSTID': delivery.vat or '',
    ...
    def action_download_excel(self):
        """Wizard formundaki butona tıklanınca dosyayı indirir."""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_url',
            'url': f"/web/content/{self.id}?model=stock.picking.export.wiz&field=excel_file&filename_field=excel_filename&download=true",
            'target': 'self',
        }
